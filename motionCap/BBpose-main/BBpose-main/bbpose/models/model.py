...
# 관절의 개수 및 모델의 깊이 관련 설정
NUM_JOINTS = 16
DEPTH = 4


# 모델 기본 블록 정의 (hourglass or softgate)
...
# 전체 네트워크 아키텍처, 입력 이미지 처리하고 특정 맵을 생성하는 역할
def network(inputs):
...
# 최종 모델 반환 함수, 입력 이미지 받아 모델 실행하고 출력을 반환
def get_model():
    if PARAMS['arch'] == 'hourglass':
